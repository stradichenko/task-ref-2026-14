...
- **Categorical** variables with few distinct values (e.g. `ARM`, `SEX`, `RACE`, `PARAMCD`) &rarr; grouping / stratification factors.
- **Continuous numeric** columns (e.g. `AVAL`, `CHG`, `BASE`, `AGE`, `BMRKR1`) &rarr; outcome or covariate in regressions, boxplots, trajectories.
- **Date / time** columns (e.g. `TRTSDTM`, `TRTEDTM`, `ADTM`) &rarr; time-on-treatment, duration calculations.
- **Censoring indicators** (`CNSR`, `EVNTDESC`) &rarr; time-to-event (survival) modelling.
- **Visit identifiers** (`AVISIT`, `AVISITN`) &rarr; longitudinal panel structure, repeated-measures models.
