...
**Duplicate detection** follows a layered strategy: exact matches, near-exact records flagged for manual review, probabilistic linkage (recordlinkage / Dedupe), and cross-modal concordance checks---genotype concordance via plink and proteomic intensity correlations---to catch subtler cases.
...
**Cross-form and cross-modal discordances** are caught by reconciling baseline diagnoses against problem lists, medications across forms, temporal consistency of key dates (onset, collection, withdrawal), and clinically recorded sex versus genetically inferred sex.

Finally, **site and personnel performance** is tracked through completeness rates, edit-check failure rates, query resolution times, and error proxies, all exposed in Grafana dashboards so that patterns can be addressed early.
...
**Grafana** serves operational needs: enrolment progress, completion rates, missingness heatmaps, query volumes, and site scorecards, all defined as code and scoped by role. **Apache Superset** supports research-oriented exploration with interactive charts, ad-hoc filtering, and row-level security managed through Keycloak. Scheduled reports are orchestrated by Airflow or Prefect Directed Acyclic Graphs (DAGs), which render parameterised scripts to PDF or HTML via Quarto, each tied to a specific script version and data cut.
